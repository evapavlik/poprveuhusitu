...
export default function WhoWeAre() {
  return (
    <Section id="co-jsme" tone="white" top="lg" bottom="md">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-start">
        <FadeUp>
          <SectionLabel>Kdo jsme</SectionLabel>
          <h2 className={`${NADPIS_1} mb-6`}>
          ...
          <p className="text-[17px] font-light leading-[1.8] text-text-muted">
          ...
          {pillars.map((p) => (
            ...
            >
              <div className="flex items-start gap-4 mb-2">
                <span className="font-cormorant text-[15px] text-brick font-semibold tracking-[0.1em] pt-[3px] shrink-0">
                  {p.num}
                </span>
                <h3 className="font-lora text-[19px] font-semibold">{p.title}</h3>
              </div>
              <p className="text-[15px] font-light leading-[1.75] text-text-muted pl-0 sm:pl-10">
                {p.text}
              </p>
            </div>
          ))}
